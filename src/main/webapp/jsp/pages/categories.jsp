...
<div class="page">
    ...
    <div class="container justify-content-center col-12 col-sm-6 mt-3">
        <h3 class="text-center p-3"><fmt:message key="category.add"/></h3>
        <form name="add_new_section" method="post" action="${absolutePath}/controller" class="needs-validation" novalidate>
            ...
            <div class="form-group mb-3">
                <label class="form-label"><fmt:message key="category.name"/></label>
                <input type="text" name="category_name" value="${param.category_name}" class="form-control" required pattern="header.[a-z_.]{1,50}">
                <c:if test="${!empty invalid_category_name}">
                    <div class="invalid-feedback-backend" style="color: red">
                        <fmt:message key="${invalid_category_name}"/>
                    </div>
                </c:if>
                <div class="invalid-feedback">
                    <fmt:message key="category.invalid_name"/>
                </div>
            </div>
        ...
        <h3 class="text-center p-3"><fmt:message key="category.update"/></h3>
        <form name="change_section_name" method="post" action="${absolutePath}/controller" novalidate>
            ...
            <label class="form-label"><fmt:message key="category.select"/></label>
            <select class="form-select" aria-label="Default select example" name="category_id">
                <c:forEach var="category" items="${applicationScope.category_list}">
                    <option value="${category.id}"><fmt:message key="${category.categoryName}"/></option>
                </c:forEach>
            </select>
            <c:if test="${! empty invalid_category}">
                <div class="invalid-feedback-backend" style="color: red">
                    <fmt:message key="${invalid_category}"/>
                </div>
            </c:if>
            <div class="invalid-feedback">
                <fmt:message key="category.invalid_category"/>
            </div>
            </br>
            <div class="form-group mb-3">
                <label class="form-label"><fmt:message key="category.new_name"/></label>
                <input type="text" name="new_category_name" value="${param.new_category_name}" class="form-control" required pattern="header.[a-z_.]{1,50}">
                <c:if test="${! empty invalid_new_category_name}">
                    <div class="invalid-feedback-backend" style="color: red">
                        <fmt:message key="${invalid_new_category_name}"/>
                    </div>
                </c:if>
                <div class="invalid-feedback">
                    <fmt:message key="category.invalid_name"/>
                </div>
                <c:if test="${! empty not_unique_new_category_name}">
                    <div class="invalid-feedback-backend" style="color: red">
                        <fmt:message key="${not_unique_new_category_name}"/>
                    </div>
                </c:if>
            </div>
            <div class="text-center mb-3">
        ...
        <h3 class="text-center p-3"><fmt:message key="category.delete"/></h3>
        <form name="delete_section_name" method="post" action="${absolutePath}/controller" novalidate>
            ...
            <c:if test="${! empty invalid_delete_category}">
                <div class="invalid-feedback-backend" style="color: red">
                    <fmt:message key="${invalid_delete_category}"/>
                </div>
            </c:if>
            <div class="invalid-feedback">
                <fmt:message key="category.invalid_category"/>
